...
const { pushTaskToQueue } = require("./taskQueue");
...
const processRequest = (rid, key, payload, handler) => {
  ...
  const localResult = pushTaskToQueue(() => orderbook.putOrder(order));
...
module.exports = {
  ...
  init: () => {
    ...
    service = serverPeer.transport("server");
    service.listen(port);
  ...
  placeOrder: async (order) => {
    ...
    try {
      ...
      throw new Error(
        localResult
          ? `Order ${localResult} was placed. Error broadcasting ${err.message}`
          : err.message
      );
    }
  },
